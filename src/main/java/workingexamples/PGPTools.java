...
public final class PGPTools {
	...
	public static PGPKeyRingGenerator createPGPKeyRingGenerator(KeyPair keyPair, int algorithm, String identity, char[] passphrase) throws Exception {
		TreeSet<String> algorithms = new TreeSet<>();
		for (Provider provider : Security.getProviders())
			for (Provider.Service service : provider.getServices())
				if (service.getType().equals("Signature"))
					algorithms.add(service.getAlgorithm());
		for (String algorithma : algorithms)
			System.out.println(algorithma);
		PGPKeyPair pgpKeyPair = new JcaPGPKeyPair(algorithm, keyPair, new Date());
		PGPDigestCalculator sha1Calc = new JcaPGPDigestCalculatorProviderBuilder().build().get(HashAlgorithmTags.SHA1);
		return new PGPKeyRingGenerator(PGPSignature.POSITIVE_CERTIFICATION, pgpKeyPair,
				identity, sha1Calc, null, null,
